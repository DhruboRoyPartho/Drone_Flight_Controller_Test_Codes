...
// working on ppm issue

// !!!!!! issues: timer channel in uno 
...
// PPM channel layout. That need be update with situation.
#define THROTTLE        3
#define ROLL            1
#define PITCH           2
#define YAW             4
...
// new function for receiver read
void read_receiver() {
    for(int i=1;i<=ChannelNumber;i++){
        ReceivedValue[i-1] = ppm.read_channel(i);
    }
}
...
void setup() {
    ...
    analogWriteFrequency(motor1, 250);
    analogWriteFrequency(motor2, 250);
    analogWriteFrequency(motor3, 250);
    analogWriteFrequency(motor4, 250);
